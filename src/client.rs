use crate::{
    error::{CreateClientError, DownloadError, UploadError, Error},
};

use google_cloud_storage::{
    client::{ClientConfig, Client as GCSClient},
    http::{
        objects::{
            upload::{UploadObjectRequest, UploadType, Media},
...
impl Client {
    pub async fn new(
        ...
        let client = GCSClient::new(ClientConfig::default()
            .with_credentials(
        ...
        O: Serialize,
    {
        ...
        match self.client.upload_object(
            ...
            serde_yaml::to_string(&object)
                .map_err(|e| UploadError::SerializeError(e))?,
        ...
        for<'de> O: Deserialize<'de>,
    {
        ...
        {
            Ok(obj_bytes) => {
                let obj_str = std::str::from_utf8(&obj_bytes)
                    .map_err(|e| DownloadError::Utf8Error(e))?;
    ...
    pub (crate) async fn upload_json<O>(
        &self,
        object: O,
        name: &str,
    ) -> Result<(), Error>
